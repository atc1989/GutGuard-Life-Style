...
**Files:** `app/app/*` · `proxy.ts` · `lib/supabase/middleware.ts` · `components/shell/*` · `components/overlays/*` · `lib/actions/member.ts` · `supabase/migrations/`
...
- [x] **E4-T01** Protect every `/app` route in middleware with a validated cookie session. `#task #epic/member`
  ...
  - Stack: Root `proxy.ts` + `createServerClient` from `@supabase/ssr`; `getUser()` (not `getSession()`) to refresh tokens; `getAll`/`setAll` cookies; redirect to `/`.
  - Done when: No cookie → 307 `/`; valid cookie → hub; tokens refresh on public routes too.

- [x] **E4-T02** Frame the hub: sidebar (≥900px), masthead, mobile segmented nav, commerce bottom bar. `#task #epic/member`
  - Route: `/app/*`
...
- [x] **E4-T03** My Health: log today’s doses, attach proof, see days of supply. `#task #epic/member`
  - Route: `/app/health`
...
- [x] **E4-T04** BASE Activation sheet: five steps with visible done state. `#task #epic/member`
...
- [x] **E4-T05** Gate **My Team** until BASE is complete. `#task #epic/member`
  - Route: `/app/team`
...
- [x] **E4-T06** Invite picker: search contacts, send invite, pending points. `#task #epic/member`
...
- [x] **E4-T07** My Story: community quotes + share wizard with consent. `#task #epic/member`
  - Route: `/app/story`
...
- [x] **E4-T08** Lock GEMA until `lifestyle_base_complete()` is true. `#task #epic/member`
  - Route: overlay `gema`
...
- [x] **E4-T09** GG-VERSE remains invitation copy — sponsor sends the link. `#task #epic/member`
...
- [x] **E4-T10** Settings: notifications + capsules/day (2–3), QR full-size. `#task #epic/member`
...
- [x] **E4-T11** Order sheet stays **mock** — no payment processing in the browser. `#task #epic/member`
